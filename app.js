...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('./service-worker.js').then((reg) => {
      // Automatic update flow: Workbox's skipWaiting()+clientsClaim() (set
      // in service-worker.js) mean a new SW takes over as soon as it's
      // installed — this listener just reloads the page once when that
      // happens, so the new assets are actually used instead of only
      // taking effect on the next manual reload.
      ...
      // Check for a new version whenever the tab regains focus, so a
      // rep who leaves the app open all day still picks up updates
      // promptly instead of waiting for the browser's ~24h check.
...
// Installability: capture the native "Add to Home Screen" prompt so it
// can be triggered from an in-app affordance instead of only the
// browser's own (often easy-to-miss) install icon.
let deferredInstallPrompt = null;
window.addEventListener('beforeinstallprompt', (e) => {
  e.preventDefault();
  deferredInstallPrompt = e;
  renderTopbar(); // re-render so the Install button appears
});
